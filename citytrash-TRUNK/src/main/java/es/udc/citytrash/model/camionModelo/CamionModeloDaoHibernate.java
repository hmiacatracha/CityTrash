...
public class CamionModeloDaoHibernate extends GenericHibernateDAOImpl<CamionModelo, Integer>
	...
	public List<CamionModelo> buscarTodosOrderByModelo(List<Integer> tiposDeBasura) {
		...
		if (tiposAux.size() > 0) {
			hql = String.format("Select " + alias + " FROM CamionModelo " + alias + " left join " + alias
					+ ".tiposDeBasura cmtb left join cmtb.pk pk left join pk.tipo tipo"
	...
	public Page<CamionModelo> buscarCamionModelo(Pageable pageable, String palabrasClaveModelo,
		...
		StringBuilder hql = new StringBuilder("Select distinct " + alias + " FROM CamionModelo " + alias
				+ " left join " + alias + ".tiposDeBasura t left join t.pk pk");
