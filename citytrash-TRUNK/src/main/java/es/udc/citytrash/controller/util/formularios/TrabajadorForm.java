...
import es.udc.citytrash.controller.util.validadores.DocumentoNoDuplicado;
import es.udc.citytrash.controller.util.validadores.EmailNoDuplicado;
...
public class TrabajadorForm {
	...
	@NotBlank(message = TrabajadorForm.NO_BLACK_MENSAJE)
	@Size(min = 2, max = 50, message = TrabajadorForm.SIZE_MENSAJE)
	...
	@NotBlank(message = TrabajadorForm.NO_BLACK_MENSAJE)
	@Email(message = TrabajadorForm.EMAIL_MENSAJE)
	@EmailNoDuplicado(message = TrabajadorForm.EMAIL_DUPLICADO_MENSAJE)
	...
	@Size(min = 9, max = 9, message = TrabajadorForm.SIZE_MENSAJE)
	@DocumentoNoDuplicado(message = TrabajadorForm.DOCUMENTO_DUPLICADO_MENSAJE)
	@Pattern(regexp = "[0-9X-Z][0-9]{7}[A-Z]", message = TrabajadorForm.DNI_NIE_MENSAJE)
	private String documento;

	@Size(min = 0, max = 50, message = TrabajadorForm.SIZE_MENSAJE)
	private String via;

	@Pattern(regexp = "^(?:[0-9]{1}|[0-9]{2}|[0-9]{3}|)$", message = TrabajadorForm.NUMERO_MENSAJE)
	private String numero;

	@Pattern(regexp = "^(?:[0-9]{1}|[0-9]{2}|[0-9]{3}|)$", message = TrabajadorForm.NUMERO_PISO_MENSAJE)
	private String piso;
	...
	@Pattern(regexp = "0[1-9][0-9]{3}|[1-4][0-9]{4}|5[0-2][0-9]{3}", message = TrabajadorForm.CP_MENSAJE)
	private String cp;

	@Past(message = TrabajadorForm.FECHA_NACIMIENTO_MENSAJE)
	@DateTimeFormat(pattern = TrabajadorForm.DATE_FORMAT_MENSAJE)
	private Date fechaNacimiento;

	@Size(min = 0, max = 255, message = TrabajadorForm.SIZE_MENSAJE)
	...
	public String toString() {
		return "RegistroForm [nombre=" + nombre + ", apellidos=" + apellidos + ", email=" + email + ", confirmarEmail="
				+ confirmarEmail + ", tipo=" + tipo + ", documento=" + documento + ", via=" + via + ", numero=" + numero
				+ ", piso=" + piso + ", puerta=" + puerta + ", cp=" + cp + ", fechaNacimiento=" + fechaNacimiento
